...
<form action="./" method="post" enctype="multipart/form-data" class="x_form-horizontal"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" />
	...
	<?php if(isset($__Context->skin_info->extra_vars) && is_array($__Context->skin_info->extra_vars) && count($__Context->skin_info->extra_vars)){ ?>
	<section class="section">
			...
			<div class="x_controls">
				<?php if($__Context->skin_info->colorset)foreach($__Context->skin_info->colorset as $__Context->key=>$__Context->val){ ?>
					...
					<div<?php if($__Context->val->screenshot){ ?> class="x_thumbnail"<?php } ?> style="display:inline-block;width:<?php echo $__Context->_width ?>px;margin-right:10px;">
						...
						<?php if($__Context->val->screenshot){ ?><img src="/rhymix/<?php echo $__Context->val->screenshot ?>" alt="<?php echo $__Context->val->title ?>" /><?php } ?>
					</div>
				<?php } ?>
			</div>
		</div><?php } ?>
...
if($__Context->val->options)foreach($__Context->val->options as $__Context->k=>$__Context->v){ ?><label for="ch_<?php echo $__Context->key ?>_<?php echo $__Context->k ?>" class="x_inline"><input type="checkbox" name="<?php echo $__Context->val->name ?>[]" value="<?php echo $__Context->v->value ?>" id="ch_<?php echo $__Context->key ?>_<?php echo $__Context->k ?>"<?php if(is_array($__Context->val->value) && in_array($__Context->v->value, $__Context->val->value)){ ?> checked="checked"<?php } ?> class="checkbox" /> <?php echo $__Context->v->title ?></label><?php }} ?>
