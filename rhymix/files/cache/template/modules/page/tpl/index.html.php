...
<form action="./" method="get" class="search x_input-append x_pull-right" style="margin-bottom:-28px" >
	...
	<?php if(count($__Context->module_category)){ ?><select name="module_category_srl" title="<?php echo $lang->module_category ?>" style="margin-right:4px">
	...
	<select name="search_target" id="search_target" style="margin-right:4px">
		<option value="s_mid"<?php if($__Context->search_target=='s_mid'){ ?> selected="selected"<?php } ?>><?php echo $lang->mid ?></option>
		<option value="s_browser_title"<?php if($__Context->search_target=='s_browser_title'){ ?> selected="selected"<?php } ?>><?php echo $lang->browser_title ?></option>
		<?php if($__Context->module_category){ ?><option><?php echo $lang->module_category ?></option><?php } ?>
	</select>
...
<!-- 목록 -->
<form action="./" method="get" id="fo_list"><input type="hidden" name="error_return_url" value="<?php echo escape(getRequestUriByServerEnviroment(), false); ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?? ''; ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" />
<table class="x_table x_table-striped x_table-hover">
	...
	<thead>
		<tr>
			...
			<th scope="col"><?php echo $lang->page_type ?></th>
	...
	<tbody>
		<?php if($__Context->page_list)foreach($__Context->page_list as $__Context->no => $__Context->val){ ?>
		<tr class="row<?php echo $__Context->cycle_idx ?>">
			...
			<td>
				...
				<?php echo $lang->virtual_site ?>
				<?php }else{ ?>
				<?php echo $lang->not_exists ?>
				<?php } ?>
				<?php }else{ ?>
			...
			<td>
				<a href="<?php echo getUrl('act','dispPageAdminInfo','module_srl',$__Context->val->module_srl) ?>"><?php echo $lang->cmd_setup ?></a>
				<i>|</i>
				<a href="<?php echo getUrl('','module','module','act','dispModuleAdminCopyModule','module_srl',$__Context->val->module_srl) ?>" onclick="popopen(this.href);return false;"><?php echo $lang->cmd_copy ?></a>
				<i>|</i>
				<a href="<?php echo getUrl('act','dispPageAdminDelete','module_srl', $__Context->val->module_srl) ?>"><?php echo $lang->cmd_delete ?></a>
				</td>
...
<div class="x_clearfix">
	<div class="x_btn-group x_pull-right">
		<a class="x_btn modalAnchor x_pull-right _manageSelected" href="#manageSelectedModule"><?php echo $lang->cmd_manage_selected_page ?></a>
	</div>
</div>
</form>
<!-- 페이지 네비게이션 -->
<?php if($__Context->page_navigation){ ?><form action="./" class="x_pagination"  style="margin:-36px 0 0 0"><input type="hidden" name="mid" value="<?php echo $__Context->mid ?? ''; ?>" />
	<input type="hidden" name="module" value="<?php echo $__Context->module ?>" />
...
jQuery(function($){
	...
	$('a.modalAnchor').click(function(){
		if($docTable.find('tbody :checked').length == 0){
			...
			alert('<?php echo $lang->msg_not_selected_page ?>');
			return false;
		}
	});
});
